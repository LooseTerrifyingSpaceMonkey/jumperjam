...
@export var screen_transistion_time = 0.5
...
func _on_button_pressed(button: ScreenButton):
	...
	match button.name:
		"TitlePlay":
			change_screen(null)
			await(get_tree().create_timer(screen_transistion_time).timeout)
			start_game.emit()
		"PauseRetry":
			change_screen(null)
			await(get_tree().create_timer(screen_transistion_time + 0.25).timeout)
